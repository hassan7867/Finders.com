    ...
    function L(a, b, c) {
        if (b.enablePagination) {
            ...
            var d = '<{0} class="actions {1}"><ul role="menu" aria-label="{2}">{3}</ul></{0}>',
                e = '<li><a href="#{0}" role="menuitem" onclick="stepForward()">{1}</a></li>',
                bc = '<li><a href="#{0}" role="menuitem" onclick="stepBackward()">{1}</a></li>',
                f = "";
            b.forceMoveForward || (f += bc.format("previous", b.labels.previous)), f += e.format("next", b.labels.next), b.enableFinishButton && (f += e.format("finish", b.labels.finish)), b.enableCancelButton && (f += e.format("cancel", b.labels.cancel)), a.append(d.format(b.actionContainerTag, b.clearFixCssClass, b.labels.pagination, f)), D(a, b, c), A(a, b, c)
        }
    }
